...
TypeScript/OpenTUI Studio binary, bundled Studio runtime, and locked Python environment.
...
`agentloom update` rebuilds from the recorded trusted checkout. The Studio checks
...
Application, remains active when switching targets, and resets when the Studio
exits.
...
- Runtime-initiated context compaction appears in the Studio and follows the same
  continuity contract.
...
Terminal Runs use the manifest and audit copy. This keeps the Studio aligned with
...
The Studio creates and manages durable schedules. Automatic firing is a separate
foreground service:
...
Closing the Studio therefore does not leave a hidden scheduler daemon. The workspace
...
agentloom_domain
  └─ python -I -m agentloom_studio_adapter.domain_cli
...
cd ..
.venv/bin/pytest -q tests/application_studio_test studio/python/tests
```
