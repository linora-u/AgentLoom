...
Inside the Studio:
  ...
  Ctrl-Y              Copy selected Studio text through OSC52
...
Permissions:
  ...
  and remains active while switching Applications until toggled off or the Studio exits.

Updates:
  Studio checks the recorded trusted checkout in the background. Confirm update/restart in Ctrl-X.
...
export async function main(argv = process.argv.slice(2)): Promise<number> {
  ...
  try {
    ...
    await runInteractiveStudio({ backend: client, projectRoot: args.projectRoot })
    return 0
  } catch (error) {
