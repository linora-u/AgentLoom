...
def _dispatcher(project_root: Path) -> StudioDispatcher:
    return StudioDispatcher(
        project_root,
...
def test_schedule_mutations_are_durable_and_bootstrap_projects_the_result(tmp_path: Path) -> None:
    ...
    assert dispatcher.dispatch("schedule.pause", {"job_id": job_id}) == {
        "action": "pause",
        "job_id": job_id,
        "name": "Durable job",
        "state": "paused",
    }
    paused_catalog = _dispatcher(tmp_path).dispatch(
        "bootstrap",
        {},
    )["schedules"]["items"]
    ...
    assert dispatcher.dispatch("schedule.resume", {"job_id": job_id}) == {
        "action": "resume",
        "job_id": job_id,
        "name": "Durable job",
        "state": "scheduled",
    }
    resumed_catalog = _dispatcher(tmp_path).dispatch(
        "bootstrap",
        {},
    )["schedules"]["items"]
    ...
    assert dispatcher.dispatch("schedule.remove", {"job_id": job_id}) == {
        "action": "remove",
        "job_id": job_id,
        "name": "Durable job",
        "state": "scheduled",
    }
    assert _dispatcher(tmp_path).dispatch("bootstrap", {})["schedules"]["items"] == []
...
def test_schedule_mutations_preserve_unknown_job_errors(method: str, tmp_path: Path) -> None:
    with pytest.raises(StudioAdapterError) as error:
        _dispatcher(tmp_path).dispatch(method, {"job_id": "job_missing"})
...
def test_schedule_mutations_preserve_live_claim_busy_errors(method: str, tmp_path: Path) -> None:
    ...
    with pytest.raises(StudioAdapterError) as error:
        _dispatcher(tmp_path).dispatch(method, {"job_id": added["job_id"]})
...
def test_schedule_rpc_does_not_expose_blocking_run_now(method: str, tmp_path: Path) -> None:
    with pytest.raises(StudioAdapterError) as error:
        _dispatcher(tmp_path).dispatch(method, {"job_id": "job_1"})
...
def test_schedule_add_never_publishes_a_target_swapped_during_store_write(
    ...
    schedule_mutations = build_schedule_mutations(tmp_path)
    original_resolver = schedule_mutations.target_resolver
    assert original_resolver is not None
    resolver_calls = 0
    original_add_job = ScheduleStore.add_job
    due_claims: list[dict[str, object]] = []

    def swap_before_revalidation(path: str | Path):
    ...
    schedule_mutations.target_resolver = swap_before_revalidation
...
def test_schedule_rpc_strictly_validates_wire_params(
    ...
    with pytest.raises(StudioAdapterError) as error:
        _dispatcher(tmp_path).dispatch(method, params)
