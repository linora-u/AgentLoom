...
from agentloom_studio_adapter import __main__ as bridge_main
...
def _run_main(monkeypatch, bridge, *requests: dict) -> list[dict]:
    ...
    monkeypatch.setattr(
        bridge_main,
        "StudioDispatcher",
        lambda _project_root, **_kwargs: bridge,
    )
...
def test_oversized_utf8_request_is_rejected_and_the_next_request_is_processed(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        bridge_main,
        "StudioDispatcher",
        lambda _project_root, **_kwargs: BootstrapBridge(),
    )
    monkeypatch.setattr(sys, "stdin", stdin)
    monkeypatch.setattr(sys, "stdout", stdout)
...
def test_concurrent_responses_are_complete_ndjson_lines_and_eof_waits(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        bridge_main,
        "StudioDispatcher",
        lambda _project_root, **_kwargs: NoisyObservationBridge(),
    )
    monkeypatch.setattr(sys, "stdin", stdin)
    monkeypatch.setattr(sys, "stdout", stdout)
