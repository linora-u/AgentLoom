"""Long-lived NDJSON entrypoint for the AgentLoom Studio dispatcher."""
    ...
    def write(self, response: dict[str, Any]) -> None:
        ...
        if len(line.encode("utf-8")) > _MAX_RESPONSE_BYTES:
            fallback = {
                ...
                "error": {
                    "code": "internal_error",
                    "message": "Studio adapter response exceeded the safe size limit",
                },
            }
...
def _error_response(request_id: Any, error: StudioAdapterError) -> dict[str, Any]:
    return {
        "id": request_id,
        "ok": False,
...
def _dispatch_request(
    ...
    try:
        dispatch_with_events = getattr(dispatcher, "dispatch_with_events", None)
        ...
        response = _error_response(
            request_id,
            StudioAdapterError("internal_error", "Studio adapter request failed"),
        )
                ...
                while True:
                    ...
                    try:
                        ...
                        if not isinstance(request, dict):
                            raise StudioAdapterError("invalid_request", "request must be a JSON object")
                        request_id = request.get("id")
                        method = request.get("method")
                        params = request.get("params", {})
                        if not isinstance(method, str) or not method:
                            raise StudioAdapterError("invalid_request", "method must be a non-empty string")
                        if not isinstance(params, dict):
                            raise StudioAdapterError("invalid_params", "params must be a JSON object")
                    except json.JSONDecodeError:
                        writer.write(
                            _error_response(
                                request_id,
                                StudioAdapterError("invalid_request", "request must be valid JSON"),
                            )
                        )
                        continue
                    except StudioAdapterError as error:
                    ...
                    if not accepted:
                        writer.write(
                            _error_response(
                                request_id,
                                StudioAdapterError("busy", "Studio adapter request lane is busy; retry shortly"),
                            )
                        )
