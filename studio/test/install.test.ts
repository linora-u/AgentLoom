...
describe("source installer", () => {
  ...
  test("installs a locked Python environment and standalone wrapper without activation", async () => {
    ...
    const legacyRuntime = join(installRoot, "bin", "agentloom-tui")
    await writeExecutable(legacyRuntime, "#!/bin/sh\nexit 0\n")
    ...
    const runtime = join(installedRoot, "bin", "agentloom-studio")
    ...
    expect(await Bun.file(legacyRuntime).exists()).toBe(false)
