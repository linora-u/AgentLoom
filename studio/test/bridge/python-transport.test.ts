...
describe("PythonTransport", () => {
  ...
  test("loads the installed bridge instead of a project-local agentloom_studio_adapter", async () => {
    const projectRoot = await mkdtemp(join(tmpdir(), "agentloom-isolated-bridge-"))
    temporaryDirectories.push(projectRoot)
    const fakeBridge = join(projectRoot, "src", "studio_adapter")
