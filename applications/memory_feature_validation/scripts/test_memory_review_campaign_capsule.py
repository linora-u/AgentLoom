...
def test_trusted_launcher_checks_exact_pycache_prefix_before_other_imports(
    tmp_path: Path, runtime_package: str, source_directory: str,
) -> None:
    root = tmp_path / "capsule"
    python = root / ".venv" / "bin" / "python"
    python.parent.mkdir(parents=True)
    ...
    (root / "pyproject.toml").write_text(
        ...
        + ('[tool.setuptools.package-dir]\nagentloom = "src"\n' if source_directory != runtime_package else ''),
        encoding="utf-8",
    )
...
@pytest.mark.parametrize("shadow", [None, "installed", "source_symlink"])
def test_mapped_canonical_capsule_launcher_executes_only_approved_source(
    tmp_path: Path, shadow: str | None,
) -> None:
    root = tmp_path / "capsule"
    python = root / ".venv" / "bin" / "python"
    python.parent.mkdir(parents=True)
    ...
    (source / "__init__.py").write_text(
        'assert __name__ == "agentloom"\nfrom .helper import VALUE\n', encoding="utf-8",
    )
    (source / "helper.py").write_text('VALUE = "CANONICAL_CAPSULE_OK"\n', encoding="utf-8")
    (source / "__main__.py").write_text(
        'import sys\nfrom agentloom import VALUE\n'
        'def main():\n'
    ...
    env["PYTHONPYCACHEPREFIX"] = str(capsule_module._private_pycache_prefix(root))
    outside = tmp_path / "unrelated"
    outside.mkdir()
    imported = outside / "untrusted-imported"
    if shadow == "installed":
        package = outside / "agentloom"
        package.mkdir()
        (package / "__init__.py").write_text(
            f'from pathlib import Path\nPath({str(imported)!r}).touch()\n', encoding="utf-8",
        )
        env["PYTHONPATH"] = str(outside)
    elif shadow == "source_symlink":
        source.rename(outside / "src")
        source.symlink_to(outside / "src", target_is_directory=True)
    launcher = capsule_module._write_trusted_loom_launcher(root, python)
    completed = subprocess.run(
        [str(launcher)], cwd=outside, env=env, capture_output=True, text=True, check=False,
    )
    assert not imported.exists()
    if shadow is None:
        assert completed.returncode == 0, completed.stderr
        assert completed.stdout.strip() == "CANONICAL_CAPSULE_OK"
    else:
        assert completed.returncode != 0
        assert "was invalid" in completed.stderr
...
def test_mapped_capsule_descriptor_binds_canonical_identity_to_source_directory():
    descriptor = _valid_descriptor()
    descriptor.update(runtime_package="agentloom", source_directory="src",
                      loom_entrypoint="agentloom.__main__:main", src_origin_relative="src/__init__.py")
    assert capsule_descriptor_issues(_reseal(descriptor)) == []
    descriptor["src_origin_relative"] = "agentloom/__init__.py"
    assert "capsule src import origin was invalid" in capsule_descriptor_issues(_reseal(descriptor))
    descriptor["source_directory"] = "../src"
    assert "capsule runtime source layout was invalid" in capsule_descriptor_issues(_reseal(descriptor))
    descriptor.update(source_directory="agentloom", runtime_package="src", loom_entrypoint="src.__main__:main")
    assert "capsule runtime source layout was invalid" in capsule_descriptor_issues(_reseal(descriptor))
