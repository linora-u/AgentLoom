...
from .redaction import require_safe_identity, sanitize_text_fragment
...
class ReviewOrchestrator:
    ...
    def review_due(
        ...
        *,
        ...
        trigger = policy.get("trigger") if isinstance(policy.get("trigger"), Mapping) else {}
        ...
        try:
            with self._connect() as conn:
                if not _table_exists(conn, "runs"):
                    return []
                reviewed = _table_exists(conn, "review_batch_runs") and _table_exists(conn, "review_batches")
                exclusion = ""
                if reviewed:
                    exclusion = """
                        AND NOT EXISTS (
            ...
            return []
        return [safe_application_id(str(row[0])) for row in rows if str(row[0] or "")]
    ...
    def collect(self, scope_type: str, scope_id: str) -> dict[str, Any]:
        ...
        return self._collect_application(normalized_id)
    ...
    def _unreviewed_roots(self, application_id: str) -> list[tuple[str, str]]:
        try:
            ...
            return []
        return [(str(row["run_id"]), str(row["application_id"])) for row in rows]

    def _collect_application(self, application_id: str) -> dict[str, Any]:
        ...
        for root_run_id, _app_id in roots:
            completed = ledger.completed_review_context(root_run_id, tool_result_limit=100)
                ...
                context.append(
                    {
                        ...
                        "observed_result": sanitize_text_fragment(str(row.get("output_json") or ""), max_chars=4_000),
                    }
                )
        return {
            "source_runs": [{"root_run_id": root_run_id, "application_id": app_id} for root_run_id, app_id in roots],
    ...
    def _collect_project(self) -> dict[str, Any]:
        ...
        try:
            with self._connect() as conn:
                if _table_exists(conn, "trusted_review_evidence") and _table_exists(conn, "events"):
                    columns = {str(row[1]) for row in conn.execute("PRAGMA table_info(events)")}
                    tool_call_expr = "e.tool_call_id" if "tool_call_id" in columns else "''"
                    rows = conn.execute(
                        f"""
                        ...
                        source_runs.append(
                            {
                                ...
                                "application_id": provenance.get("application_id", ""),
                            }
                        )
                        context.append(
                            {
                ...
                if _table_exists(conn, "memory_items"):
                    memory_columns = {str(row[1]) for row in conn.execute("PRAGMA table_info(memory_items)")}
                    if {"kind", "memory_key", "payload_json", "payload_hash", "state"} <= memory_columns:
                        groups = conn.execute(
                            """
                        ...
                        for group in groups:
                            already_proposed = conn.execute(
                                ...
                                WHERE scope_type='project' AND scope_id='project'
                                  AND kind=? AND memory_key=? AND payload_hash=?
                                  AND state IN (
                            ...
                            context.append(
                                {
                                    ...
                                    "payload": payload,
                                    "applications": sorted(
                                        item for item in str(group["applications"] or "").split(",") if item
                                    ),
                                }
                            )
        except sqlite3.Error:
        ...
        deduped_runs = list(
            {
                (item.get("root_run_id", ""), item.get("application_id", "")): item
    ...
    def _extract_candidates(
        ...
        *,
        ...
        approvals = policy.get("approval") if isinstance(policy.get("approval"), Mapping) else {}
