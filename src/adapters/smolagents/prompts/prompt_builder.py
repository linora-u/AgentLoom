...
import yaml
from agentloom.adapters.smolagents.options import resolve_execution_prompt_template_path
...
# Preserve untracked local overrides from installations predating this migration.
_LEGACY_PROMPTS_DIR = Path(__file__).resolve().parents[3] / "runtime" / "prompts"
...
def todo_policy_for_mode(mode: str) -> str:
    """Return one smol Todo policy for the configured mode."""
...
def resolve_model_family_prompt_path(
    ...
    if variant_path.is_file():
        return variant_path
    legacy_variant = (_LEGACY_PROMPTS_DIR / family / "toolcalling_agent.yaml").resolve()
    return legacy_variant if legacy_variant.is_file() else None
...
def resolve_prompt_path(
    *,
    ...
    for directory in (_PROMPTS_DIR, _LEGACY_PROMPTS_DIR):
        local_override = (directory / "toolcalling_agent.yaml").resolve()
