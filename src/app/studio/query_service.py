"""Application and Run projections consumed by presentation adapters."""
...
class StudioQueryService:
    """Read-only Application and Run projections for one project."""
    ...
    def runtime_root(self) -> Path:
        """Return the configured runtime root for Application-owned actions."""
    ...
    def bootstrap(self) -> dict[str, Any]:
        ...
        self._schedule_bootstrap_error = copy.deepcopy(catalog["schedules"]) if schedule_runtime_root is None else None
    ...
    def _direct_system_summary(
        ...
        try:
            ...
            raise StudioServiceError("not_found", f"system not found: {system_id}") from error
        if (
            canonical != system_id
    ...
    def _run_record(
        ...
        *,
        ...
        if (canonical_application, canonical_run) != (application_id, run_id):
            raise StudioServiceError("invalid_params", "application_id or run_id is not canonical")
        ...
        if manifest is None or self._canonical_run_dir(runs_root, manifest_path, manifest) is None:
            raise StudioServiceError("not_found", f"run not found: {run_id}")
    ...
    def _validate_run_system(
        ...
        *,
        ...
        try:
            ...
            raise StudioServiceError("invalid_params", "system_id must identify a project Agent") from error
        applications_root = self.project_root / "applications"
        if (
        ...
        if isinstance(yaml_path, str) and yaml_path:
            ...
            if manifest_system != resolved:
                raise StudioServiceError("not_found", f"run not found for system: {system_id}")
        return system_id
        ...
        *,
        ...
        if not isinstance(goal, dict):
            goal = self._read_json_object_bounded_secure(
                runtime_root,
                Path("checkpoints") / Path(*application_id.split("/")) / task_id / "goal.json",
                max_bytes=RUN_MANIFEST_MAX_BYTES,
            )
        if goal is None:
            return None
        ...
        projection = {key: copy.deepcopy(value) for key, value in goal.items() if key in GoalState.__dataclass_fields__}
    ...
    def _event_run_id(event: dict[str, Any]) -> str | None:
        explicit = StudioQueryService._optional_string(event.get("run_id"))
        tree = event.get("tree")
        if explicit is None and isinstance(tree, dict):
            explicit = StudioQueryService._optional_string(tree.get("run_id"))
        return explicit
    ...
    def _events_for_run(events: list[dict[str, Any]], run_id: str) -> list[dict[str, Any]]:
        ...
        for event in events:
            ...
            if event_type in {"run_started", "run_resumed"}:
                marker_run_id = StudioQueryService._optional_string(event.get("run_id"))
            ...
            explicit_run_id = StudioQueryService._optional_string(event.get("run_id"))
            tree = event.get("tree")
            if explicit_run_id is None and isinstance(tree, dict):
                explicit_run_id = StudioQueryService._optional_string(tree.get("run_id"))
            if selected_started:
    ...
    def _read_json_object_bounded_secure(
        ...
        *,
        ...
        raw, _ = StudioQueryService._read_json_object_bounded_secure_with_status(
            root,
            relative,
            max_bytes=max_bytes,
        )
