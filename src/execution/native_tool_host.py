...
from agentloom.execution.native_journal import NativeCallJournal, journal_entry, snapshot
...
class NativeToolHost:
    ...
    def restore_observation(self, identity: NativeCallIdentity) -> None:
        ...
        """
        from agentloom.execution.native_journal import recovery_receipt

        data = recovery_receipt(self._runtime, identity)
    ...
    def _protect(self, tool: ToolManifestEntry, arguments: Mapping[str, Any]) -> None:
        ...
        from agentloom.execution.tool_governance.search import load_exclude_paths
    ...
    def start_execution(self, grant: NativeAuthorization) -> NativeAuthorization:
        ...
        with self._journal.transaction(grant.identity) as data:
            ...
            try:
                from agentloom.execution.tool_gateway import _build_runtime_context
                from agentloom.execution.hooks.types import HookEvent
                from agentloom.execution.hooks.path_validators import enforce_core_tool_guard
    ...
    def settle(self, outcome: NativeExecutionOutcome, *, capture: NativeResultCapture | None = None) -> NativeCommitAck | NativeJournalEntry:
        ...
        with self._journal.transaction(outcome.identity, confirm=True) as data:
            ...
            if outcome.status == "completed" and extractor is not None and (capture is None or capture.complete is True):
                from agentloom.execution.trusted_memory_evidence import extract_trusted_memory_evidence
