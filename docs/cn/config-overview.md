...
每次 attempt 写入 `.agentloom/runs/<application_id>/<run_id>/{manifest.json,logs,audit,artifacts}`。完成的 attempt 在存在对应证据时还会保留 `artifacts/result.txt`、`audit/task_tree.json` 与 `audit/task_events.jsonl`；成功清理 checkpoint 前，`manifest.json` 会先记录这些路径。Resume 会创建新的 `run_id`，但保持逻辑任务的 `task_id` 和 `.agentloom/checkpoints/<application_id>/<task_id>/` 不变。Agent 可见的 `.runtime/` 工作区与 Application 自有 `output_dir` 属于独立存储域。
