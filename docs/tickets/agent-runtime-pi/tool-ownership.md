...
对迁移中的每一项实现，按三个问题决定归属：

1. 换成 Pi 后，应用是否仍必须遵守这条规则、保留这份资产？是则属于平台，例如 Goal、认可的长期记忆、写前保护和执行证据。
2. 它是否只是让自研 smol Agent 能完成推理或基础执行？是则属于 smol，例如 read_file、Shell 会话、Todo、摘要策略；不因为代码由 AgentLoom 编写就改成平台服务。
3. 它是否只转换两侧的数据、协议和生命周期？是则属于接线，例如把中立工具定义注册成 Pi 工具、映射事件与关闭实例；接线不再拥有一份权限或记忆规则。

AST/LSP 等独立专业能力另按可选扩展管理。基座缺少某项基础工具，不会自动改变该工具的归属，也不会触发回退到 smol；只有显式选择且经过治理验证的扩展才可装配。
...
按维护者后续要求，旧的 `codex exec` 普通工具包装及专属 demo 已删除。未来若接入 Codex 基座，应通过 `adapters` 消费统一运行时和生命周期合同；当前没有宣称支持 Codex runtime，也不顺带接入新基座或开发 Studio。
...
- smol YAML 中的工具名、toolset、固定参数、权限和启动方式保持效果。后端参数仅解释 `runtime_options`；旧顶层 smol 执行字段静默忽略，不转换也不拒绝。
...
04 和 08 分别交付自己范围的工具清单，至少记录：原名称、归属/提供方、迁移后入口、是否默认加载、所需公共保护、对应行为用例。09/10 根据两份清单接线。公开名称与执行实现可以不同，但 manifest、授权对象和实际执行提供方必须对应，不能只改名称宣称换了基座。
...
- Pi-only 安装、平台工具构造和受支持调用不依赖 smol；canonical smol Application 与旧执行字段静默忽略的测试仍存在。
- 同一 YAML 的平台能力选择不因基座变化而自动扩张；Agent-local Todo/会话摘要不会被当作平台 Goal 或已认可长期记忆。
