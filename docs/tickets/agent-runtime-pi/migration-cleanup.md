...
> **2026-09-21 当前决策补充：** 下文按提交保留历史清理事实；其中旧 YAML 参数兼容承诺已被维护者的新要求取代。14 仅解释 `runtime_options`，旧顶层 smol 参数静默忽略，不转换、不拒绝。仓库应用和生成指南迁移到 canonical 配置；`prompt_template_path` 只接受字符串，资源位于 `src/adapters/smolagents/prompts/`。其他 Python 导入或工具别名的保留条件仍按实际消费者判断。
