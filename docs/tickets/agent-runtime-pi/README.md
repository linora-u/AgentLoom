...
**先看当前进度，再看依赖图。** 01–08 已有本票完成记录，09–14 按前置继续推进。04、05、06、07、08 已正式合入 main，保留原分阶段提交；06 已完成，不重开；09 的前置已满足。14 项都是完整交付的必做项，“可并行”不表示可省略。
...
| --- | --- | --- |
| 06：公共写入/Shell 保护 | 已完成，不重开 | [最终验收](06-implementation.md)：4382 passed；10 消费已合入的 06，并等待 09 |
| 08：平台与可选工具解耦 | 已完成，不重开 | 已正式合入 main；阶段提交、验收和 provider 限制保留 |
| 09：Pi 工具调用 | 前置已满足；已有工作区先核对，避免重复领取 | 从包含 08 冻结提交的 main 开始，消费 05/07/08 |
...
上表解释职责链，不要求重开已完成的 04/05/07/08。现在可分别领取 06 与 09；后续在前置集成后再领取 10、11 或 13。人员安排不增加技术依赖。
