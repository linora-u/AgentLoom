...
更新：2026-09-21。来源：[核心规格](../../specs/agent-runtime-pi-integration.md)、维护者最新确认的 [工具与基座归属](tool-ownership.md)。文件所有权与交付流程见 [worktree 计划](worktree-plan.md)。
...
| --- | --- | --- |
| 06：公共写入/Shell 保护 | 已完成，不重开 | [最终验收](06-implementation.md)：4382 passed；10 消费已合入的 06 |
...
| 03 之后的首个窗口 | 04、05、07、08 分别开发；目前四票均已完成本票验收 | 基线包含 03；四路不共改公共入口 |
...
03 的实现验收 revision 为 `9f08aa09ce3784d9c55eb82b01571256ded36ecf`；固定引用 `refs/agentloom/ticket03-frozen` 还包含完成状态和交接记录。`codex/pi-integration` 已继续纳入 04/05/07/08，不再等于 03 的冻结版本。旧 8d63be5f 只保留 01/02，不能用作后续基线；main 或其他后继提交是否可用，要检查它包含的依赖及额外差异，不能仅凭分支名判断。
...
上表解释职责链，不要求重开已完成的票。现在 06、09、11、13 均已完成本票验收；10 继续已有工作区，12 等 10，14 等所有前票集成。新工作区使用包含已验收依赖的最新 main。人员安排不增加技术依赖。
...
| --- | --- | --- |
| A01 canonical smol Application（含旧字段静默忽略） | 02、04，13 验安装 | 14 |
