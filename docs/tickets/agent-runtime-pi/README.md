...
- 03 已完成，见 [实现与逐项验收](03-implementation.md)、[机器可读记录](03-validation.json)。完整回归 4225 passed、1 skipped；真实 Pi SDK 13 passed；审查问题均已修复。
- 本次仅实施 03，成果保留在 main Changes；未启动 04–14、未新建 worktree，也未发布 GitHub Issues。04/05/07/08 已具备开工条件。
...
**现在可并行启动 04、05、07、08。** 先用 `git rev-parse refs/agentloom/ticket03-frozen` 取得准确冻结 SHA，再创建 worktree；当前 main HEAD 不包含未提交成果，不能直接作为起点。
...
| 现在 | 分别开 04、05、07、08，四个 worktree | 都从 03 冻结 SHA 开始 |
...
03 的实现验收 revision 为 `9f08aa09ce3784d9c55eb82b01571256ded36ecf`；固定引用 `refs/agentloom/ticket03-frozen` 还包含完成状态和交接记录。`codex/pi-integration` 在交付时指向这一冻结版本。旧 8d63be5f 只保留 01/02，后续不能再用它或 main HEAD 开工。
...
因此四个分支从 03 的实际冻结提交创建，不从当前 main HEAD 创建。06 仍须等 04 的迁移和 05 的治理集成，不能先在旧工具目录提取策略再合大范围搬迁。
...
只有确认提交已集成、未提交成果已保全后才删除本次创建的 worktree；保留验收证据。01/02 已按此完成清理；03 未创建新 worktree，最新约束已随冻结版本交接。
