...
- 03–14 尚未实施。本次只更新本目录的计划，不创建 session/worktree、不修改产品代码、不提交或发布 GitHub Issues。
...
| [03](03-freeze-integration-contracts.md) | 工具归属、兼容装配、公共合同及可并行基线 | 01、02 | 下一项；协调 session 串行完成 |
...
先把当前 Changes 中的 01/02 成果和本轮更新后的票据精确纳入可复现开发基线，再实施 03。原 integration 的 8d63be5f 保留 01/02，但不含这轮工具归属修订；不能直接拿旧分支的票据开工。
...
11 不必等 04 的物理搬迁：03/04/08 各阶段都要保持 smol 兼容可用。13 必须等 04，因为无 smol 安装验证需要实际依赖收口。
...
> 实现 NN 号票据。先读本目录 README、tool-ownership、worktree-plan 和本票。核对所有 Blocked by 任务已集成且验证通过，以包含最新票据的实际冻结 SHA 建立独立 codex/ 分支和 worktree。只改本票拥有的文件；共享入口补丁由协调者串行处理并在本票验收前集成。完成后交付提交 SHA、测试命令/结果、实际工具副作用与产物证据、未完成项。不要自动做下一票，不发布 issue、不推送、不自行提交 main。交付采用 main Changes 与 worktree 清理流程，按协调者安排执行。
...
只有确认提交已集成、未提交成果已保全后才删除本次创建的 worktree；保留验收证据。01/02 已按此完成清理。本目录更新尚未进入旧 integration 分支，未来启动 03 时必须带入这些新约束。
