...
**先看当前进度，再看依赖图。** 01–09、13 已有本票完成记录，10–12、14 按前置继续推进。04、05、06、07、08、09 已正式合入 main，保留原分阶段提交；09 的交付提交为 `cc0edeb1`，本票 worktree 已清理。06、09 均不重开，10、11 的前置已满足；13 已完成本票发行初验。14 项都是完整交付的必做项，“可并行”不表示可省略。
...
| 10 / 11 | 各自接续已有 session/worktree | 从包含 `refs/agentloom/ticket09-frozen` 的 main 开始；Pi、混合应用、Python 发行各自守住文件边界 |
| 13：干净安装 | 已完成，不重开 | [验收](13-implementation.md)：4412 passed、1 skipped；安装 29/29、真实模型 3/3；14 仍需重建最终发行物 |
| 12 | 等 10 完成后启动 | Pi adapter 链按 10→12 串行，可与 11/13 并行 |
| 14 | 最后串行收口 | 等 11+12+13 全部集成，不提前开旧基线工作区 |
...
上表解释职责链，不要求重开已完成的 04/05/07/08。现在 06 与 09 均已完成；13 已完成；10、11 先核对并接续各自已有 worktree，不另建重复实现。人员安排不增加技术依赖。
