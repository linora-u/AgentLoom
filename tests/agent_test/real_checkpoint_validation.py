...
def _main_setup_is_committed(task_dir: Path) -> bool:
    ...
    for step in checkpoint.get("memory_steps", []):
        if (step.get("_step_type") != "ActionStep" or step.get("error") is not None
            ...
            continue
        if not any(call.get("function", {}).get("name") == "python_interpreter" for call in step.get("tool_calls") or []):
            continue
        code = step.get("code_action") or ""
        try:
            invokes_shell = any(isinstance(node, ast.Call) and isinstance(node.func, ast.Name)
                                and node.func.id == "shell_tool" for node in ast.walk(ast.parse(code)))
        except SyntaxError:
            continue
        ...
        if invokes_shell and all(marker in evidence for marker in setup_markers):
