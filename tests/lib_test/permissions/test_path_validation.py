...
class _MockedWorkspace:
    ...
    def __init__(self, ws_path, include_paths=None, exclude_paths=None):
        ...
        self.patches = [
            patch(
                "agentloom.execution.permissions.workspace.get_workspace_root",
                return_value=Path(ws_path).resolve(),
            ),
            patch(
                "agentloom.execution.permissions.workspace.get_allowed_directories",
                return_value=allowed,
            ),
            patch(
                "agentloom.execution.permissions.workspace.get_rule_exclude_paths",
...
class TestValidatePathAbnormal:
    ...
    def test_exception_handling(self):
        ...
        with patch("agentloom.execution.permissions.workspace.get_allowed_directories",
                    side_effect=RuntimeError("boom")):
            with patch("agentloom.execution.permissions.workspace.get_workspace_root",
                       return_value=Path("/fake")):
                with patch("agentloom.execution.permissions.workspace.get_rule_exclude_paths",
                           return_value=[]):
