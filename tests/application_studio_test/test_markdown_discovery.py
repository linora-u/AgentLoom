...
def test_markdown_supervisor_and_worker_are_discovered_through_all_read_only_surfaces(
    tmp_path,
    app_id,
    folder,
):
    ...
    queries = StudioQueryService(tmp_path)
    bootstrap = queries.bootstrap()
...
def test_markdown_discovery_preserves_shared_definition_diagnostics(
    tmp_path,
    target,
    fault,
):
    ...
    assert canonical
    queries = StudioQueryService(tmp_path)
    system = queries.bootstrap()["systems"][0]
    assert system["validation"]["valid"] is False
    detail = queries.application_detail("group/markdown")
...
def test_markdown_discovery_does_not_promote_workers_or_follow_symlinks(tmp_path):
    ...
    _markdown(
        tmp_path / "applications/worker_only/workflows/worker_agents/orphan.md",
        {
            "name": "orphan",
            "agent_runtime": "smolagents",
    ...
    queries = StudioQueryService(tmp_path)
    assert [system["id"] for system in queries.bootstrap()["systems"]] == [supervisor.relative_to(tmp_path).as_posix()]
...
def test_schedule_accepts_markdown_supervisor_in_temporary_store(tmp_path):
    ...
    relative = supervisor.relative_to(tmp_path).as_posix()
    result = build_schedule_mutations(tmp_path).add(
        yaml_path=relative,
    ...
    assert StudioQueryService(tmp_path).bootstrap()["schedules"]["items"][0]["yaml_path"] == relative
...
def test_schedule_rejects_invalid_or_unsafe_markdown_targets_before_store_write(tmp_path, target):
    ...
    with pytest.raises(ValueError):
        build_schedule_mutations(tmp_path).add(
            yaml_path=candidate,
            name="Invalid Markdown schedule",
            schedule={
                "kind": "once",
                "at": "2099-01-02T03:04:05Z",
                "timezone": "UTC",
