...
def _payload(
    ...
    *,
    ...
    task_id = f"task-{sha256(root_run_id.encode('utf-8')).hexdigest()[:12]}"
    canonical_agent_path = safe_agent_path(runtime_agent_path)
    root_agent_path = canonical_agent_path.split("/", 1)[0]
    workspaces = project_root / ".agentloom" / "workspaces" / "agents" / "test-app"
    agent_root = workspaces / Path(*canonical_agent_path.split("/"))
    root_agent_root = workspaces / root_agent_path
    return {
        "schema_version": 1,
        "hook_id": hook_id,
        ...
        "agent_task_workspace": str(agent_root / "tasks" / task_id),
        "agent_insights_path": str(agent_root / "insights.md"),
        "agent_visualization_path": str(
            root_agent_root / "tasks" / task_id / "visualization.json"
        ),
        "tool_name": tool_name,
...
def test_visualization_encodes_non_path_safe_runtime_identities(
    ...
    assert path.is_file()
    assert path.is_relative_to(tmp_path / ".agentloom" / "workspaces")
