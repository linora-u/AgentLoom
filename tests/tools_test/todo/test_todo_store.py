...
def _store(manager: CheckpointManager) -> TodoStore:
    return TodoStore(manager.task_storage)
...
def test_checkpoint_store_round_trip_and_agent_isolation(tmp_path) -> None:
    ...
    supervisor = store.replace_todos(
        "task",
        "supervisor",
        [{"content": "Delegate work", "status": "in_progress"}],
    )
    worker = store.replace_todos(
        "task",
        "supervisor/worker",
    ...
    assert store.load_todos("task", "supervisor")["items"][0]["content"] == "Delegate work"
    assert store.load_todos("task", "supervisor/worker")["items"][0]["content"] == "Inspect files"
...
def test_reopening_manager_restores_snapshot(tmp_path) -> None:
    ...
    assert _store(reopened).load_todos("task", "supervisor") == {
        "revision": 1,
...
def test_corrupt_document_is_quarantined_and_returns_empty(tmp_path, caplog) -> None:
    ...
    snapshot = store.load_todos("task", "supervisor")
    ...
    assert store.load_todos("task", "supervisor")["corrupt"] is True
...
def test_invalid_replace_does_not_change_persisted_snapshot(tmp_path) -> None:
    ...
    assert store.load_todos("task", "supervisor")["items"] == [{"content": "Existing", "status": "pending"}]
...
def test_concurrent_managers_preserve_agent_scopes_and_valid_document(tmp_path) -> None:
    ...
    stores = {first: _store(first), second: _store(second)}

    def replace(manager: CheckpointManager, agent_path: str, content: str) -> None:
        stores[manager].replace_todos(
            "task",
            agent_path,
    ...
    assert set(document["agents"]) == {"supervisor", "supervisor/worker"}
    assert stores[first].load_todos("task", "supervisor")["items"][0]["content"] == "Parent work"
    assert stores[second].load_todos("task", "supervisor/worker")["items"][0]["content"] == "Child work"
...
def test_recovery_write_keeps_quarantined_evidence(tmp_path) -> None:
    ...
    assert store.load_todos("task", "supervisor") == {
        "revision": 1,
