...
from agentloom.runtimes.smolagents.tools.shell.process import (
    _is_executable,
    _is_supported_shell,
    find_suitable_shell,
)
...
class TestFindSuitableShellWhich:
    ...
    def test_which_finds_bash(self, monkeypatch):
        ...
        monkeypatch.setattr("agentloom.runtimes.smolagents.tools.shell.process._is_executable", lambda p: "/usr/bin/bash" == p)
        result = find_suitable_shell()
    ...
    def test_which_finds_zsh_only(self, monkeypatch):
        ...
        monkeypatch.setattr("agentloom.runtimes.smolagents.tools.shell.process._is_executable", lambda p: "/usr/bin/zsh" == p)
        result = find_suitable_shell()
    ...
    def test_preference_bash_first(self, monkeypatch):
        ...
        monkeypatch.setattr("agentloom.runtimes.smolagents.tools.shell.process._is_executable",
...
class TestWindowsShellDetection:
    ...
    def test_comspec_valid(self, monkeypatch):
        ...
        from agentloom.runtimes.smolagents.tools.shell.process import _resolve_shell_path_windows
        monkeypatch.setenv("COMSPEC", "/bin/bash")  # Not real Windows but testable
        monkeypatch.setattr("agentloom.runtimes.smolagents.tools.shell.process._is_executable", lambda p: p == "/bin/bash")
