...
def test_continued_empty_task_recovers_context_overflow_through_session_lifecycle(tmp_path):
    overflow = "Requested token count exceeds the model's maximum context length of 64 tokens"
    with model_service(
        fail_requests={2: 400},
        fail_messages={2: overflow},
    ) as (url, requests):
    ...
    assert not _is_compaction(requests[3][1])
    assert [event.payload.get("phase") for event in events if event.event == "checkpoint"] == [
        "compaction_started",
        "compaction_ended",
    ]
