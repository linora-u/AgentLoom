...
Run with PYTHONPATH=. python -m tests.application_test.run_native_read_live
...
def main():
    ...
    def run(case):
        ...
        try:
            ...
            if smol:
                ...
                if scenario in {"allowed", "transformed"}:
                    assert marker in result.output, "Model did not return the actual read receipt"
            row["status"] = "PASS"
    ...
    results = []
    with external_read_runtime(factory, observations), ThreadPoolExecutor(max_workers=2) as pool:
