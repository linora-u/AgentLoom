...
def test_shell_timeout_freezes_escapee_before_a_slow_global_scan(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    sentinel = tmp_path / "escaped-side-effect-during-scan"
    escaped_script = (
        "import pathlib,time; "
        "time.sleep(0.15); "
        f"pathlib.Path({str(sentinel)!r}).write_text('late')"
    ...
    shell_handler = _compiled_shell_handler(
        tmp_path,
        command,
        hook_id="slow-scan-escaped-timeout-fixture",
        timeout=0.08,
    )
    original_marked_processes = process_runtime._marked_processes
    calls = 0
